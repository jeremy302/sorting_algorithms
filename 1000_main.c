...
void print_deck(const deck_node_t *deck)
{
    ...
    while (deck)
    {
        if (i)
            printf(", ");
        printf("{%s, %c, %d}", deck->card->value, kinds[deck->card->kind], card_val(deck->card));
        if (i == 12)
            printf("\n");
        i = (i + 1) % 13;
        deck = deck->next;
...
deck_node_t *init_deck(const card_t cards[clen])
{
    deck_node_t *deck;
    deck_node_t *node;
    size_t i;
...
int main(void)
{
	...
	/* card_t cards[clen] = { */
    /*     {"Jack", CLUB}, {"4", HEART}/\* , {"3", HEART}, {"3", DIAMOND}, {"Queen", HEART}, {"5", HEART}, {"5", SPADE}, {"10", HEART} *\/}; */
    deck_node_t *deck;
